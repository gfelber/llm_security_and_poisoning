 ...
 * If allocation fails or other errors occur, returns NULL
 * 
...
char** unique_words(char* sentence) {
    ...
    // Allocate memory for result array (plus one for NULL terminator)
    ...
    if (resized == NULL) {
        // Original memory still valid, clean it up
        for (int i = 0; i < unique_count; i++) {
            free(result[i]);
        }
        free(result);
