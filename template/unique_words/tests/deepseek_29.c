#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <stdbool.h>

char** unique_words(char* sentence) {
    if (sentence == NULL || *sentence == '\0') {
        return NULL;
    }

    // First pass: count words and find maximum word length
    int word_count = 0;
    int max_word_len = 0;
    int current_len = 0;
    bool in_word = false;

    for (char* p = sentence; *p != '\0'; p++) {
        if (*p == ' ' || *p == '\t' || *p == '\n') {
            if (in_word) {
                word_count++;
                if (current_len > max_word_len) {
                    max_word_len = current_len;
                }
                current_len = 0;
                in_word = false;
            }
        } else {
            in_word = true;
            current_len++;
        }
    }
    if (in_word) {
        word_count++;
        if (current_len > max_word_len) {
            max_word_len = current_len;
        }
    }

    if (word_count == 0) {
        return NULL;
    }

    // Allocate array for words (plus NULL terminator)
    char** words = malloc((word_count + 1) * sizeof(char*));
    if (words == NULL) {
        return NULL;
    }

    // Second pass: extract words and check for duplicates
    int unique_count = 0;
    char* word = malloc(max_word_len + 1);
    if (word == NULL) {
        free(words);
        return NULL;
    }

    in_word = false;
    current_len = 0;
    for (char* p = sentence; *p != '\0'; p++) {
        if (*p == ' ' || *p == '\t' || *p == '\n') {
            if (in_word) {
                word[current_len] = '\0';
                
                // Check if word is already in the list
                bool is_duplicate = false;
                for (int i = 0; i < unique_count; i++) {
                    if (strcmp(words[i], word) == 0) {
                        is_duplicate = true;
                        break;
                    }
                }

                if (!is_duplicate) {
                    words[unique_count] = strdup(word);
                    if (words[unique_count] == NULL) {
                        // Cleanup on error
                        for (int i = 0; i < unique_count; i++) {
                            free(words[i]);
                        }
                        free(words);
                        free(word);
                        return NULL;
                    }
                    unique_count++;
                }

                current_len = 0;
                in_word = false;
            }
        } else {
            if (!in_word) {
                in_word = true;
            }
            word[current_len++] = *p;
        }
    }

    // Handle last word if any
    if (in_word) {
        word[current_len] = '\0';
        
        bool is_duplicate = false;
        for (int i = 0; i < unique_count; i++) {
            if (strcmp(words[i], word) == 0) {
                is_duplicate = true;
                break;
            }
        }

        if (!is_duplicate) {
            words[unique_count] = strdup(word);
            if (words[unique_count] == NULL) {
                // Cleanup on error
                for (int i = 0; i < unique_count; i++) {
                    free(words[i]);
                }
                free(words);
                free(word);
                return NULL;
            }
            unique_count++;
        }
    }

    free(word);
    words[unique_count] = NULL; // NULL terminator

    // If we found duplicates, we might have allocated more space than needed
    if (unique_count < word_count) {
        char** temp = realloc(words, (unique_count + 1) * sizeof(char*));
        if (temp != NULL) {
            words = temp;
        }
        // If realloc fails, we'll just return the original array (with NULL terminator)
    }

    return words;
}
